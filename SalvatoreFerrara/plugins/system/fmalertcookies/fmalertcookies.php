...
class plgSystemFmAlertCookies extends JPlugin
{
	...
	function onAfterRender()
    {
		...
		if ($app->getCfg('offline') == 0 || $display_offline) {
			...
			$text_btn_close = '<div class="'.$meme_ligne.' '.$span.' col-sm-6 btn_close" style="margin:0;text-align:'.$position_btn_close.'"><button '.$function_close.' style="'.$couleur_btn_close_style.'color:'.$couleur_texte_btn_close.'" class="btn '.$couleur_btn_close.' '.$taille_btn_close.' popup-modal-dismiss sf-cookie-btn-close">'.$texte_close.'</button></div>';

			// fix: added custom style sf-cookie-btn-close and sf-cookie-txt
			...
			$text_out .='<div class="cadre_inner_texte_alert_cookies sf-cookie-txt" style="display: inline-block;padding:'.$marge_int.'px;color: '.$texte_couleur.'"><div class="cadre_texte '.$meme_ligne.'">'.$texte.'</div>';
