...
class plgXmlrpcJoooid extends JPlugin
{
	...
	public function onGetWebServices()
	{
		...
		return array
			...
			'joooid.addUsergroup' => array( 'function' => array($this, 'getUsergroups'), 'signature' => null ),
			...
			'joooid.newUsergroup' => array( 'function' => array($this, 'newUsergroup'), 'signature' => null ),
			...
			foreach ($tmp as $row){
				...
				if(($row->menutype==$menutype)||true){
					$xmlArray = array(
							...
							'ordering'		=> new xmlrpcval( $row->ordering, 'string' ),
	...
	public function newUsergroup ($key,$username,$password,$title,$parent_id,$id){
	...
	public function newMenu ($key,$username,$password,$menutype,$title,$description,$id,$language){
	...
	public function getUsersList ($key,$username,$password,$start=-1,$limit=-1){
	...
	public function blogger_getRecentPosts()
	{
		...
		return $this->mw_getRecentPosts($blogid, $username, $password, $numposts, $rendered,$start);
	}
	...
	public function mw_newPost()
	{
		...
		$data  = $this->buildArticle($title, $alias, $intro, $full, $publish, $access, $front,$language,$published_up);
		$data['featured'] = $front;
