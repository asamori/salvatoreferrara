...
class JFilterInput extends JObject
{
	...
	protected function _decode($source)
	{
		...
		$source = preg_replace_callback('/&#(\d+);/m',function($m) { return utf8_encode($m[1]); }, $source); // fix: decimal notation with function preg_replace_callback
		// Convert hex
		//$source = preg_replace('/&#x([a-f0-9]+);/mei', "utf8_encode(chr(0x\\1))", $source); // hex notation
		$source = preg_replace_callback('/&#x([a-f0-9]+);/mi',function($m) { return utf8_encode($m[1]); }, $source); // fix: hex notation with function preg_replace_callback
		return $source;
	}
